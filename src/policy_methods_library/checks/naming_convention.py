...
def check_naming_convention(client: Optional[GitHubRestClient] = None, repository_name: Optional[str] = None, data: Optional[dict] = None) -> dict:
    ...
    if data is None:
        if client is None:
            raise ValueError("A GitHubRestClient instance must be provided if data is not passed directly.")
        if repository_name is None:
            raise ValueError("A repository name must be provided if data is not passed directly.")
        
        try:
            response = client.make_request("GET", f"/repos/{client.owner}/{repository_name}")
